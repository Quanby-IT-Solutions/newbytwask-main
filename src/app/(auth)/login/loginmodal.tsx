...
const _LoginForm: React.FC<LoginFormProps> = ({ onSubmit, error }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6 max-sm:gap-3">
      {error && <p className="text-red-500">{error}</p>}
      <div className="flex flex-col">
        <label htmlFor="email" className='pb-1 text-gray-500 text-[15px] max-sm:text-[12px]'>Email</label>
        <input
          type="email"
          placeholder="Email"
          value={email}
      ...
      <div className="flex flex-col">
        <label htmlFor="email" className='pb-1 text-gray-500 text-[15px] max-sm:text-[12px]'>Password</label>
        <input
          type="password"
      ...
      <button type="submit" className="w-fit place-self-center bg-[#60A5FA] text-white text-xl max-sm:text-[15px] font-medium py-2 px-14 rounded-lg hover:bg-blue-600">
        Login
      </button>
    </form>
  );
...
const _LoginModal: React.FC<ModalBackdropProps> = ({ open, onClose, toggleRegisterModal }) => {
  ...
  return (
    <main onClick={onClose} className={`fixed inset-0 flex justify-center items-center transition-all duration-300 ease-in-out transform ${open ? "visible bg-black/75 backdrop-blur-md" : "invisible"}`}>
      <div className={`flex justify-center content-center w-fit mx-10 relative transition-all duration-300 rounded-lg ease-in-out transform  bg-blue-400  ${modalScale}`}>
        <div onClick={(e) => e.stopPropagation()} className={`container w-fit h-fit mx-auto p-24 max-sm:p-14 font-poppins rounded-l-lg rounded-r-3xl max-[1120px]:rounded-r-lg bg-white transition-all duration-300 ease-in-out transform`}>
          <p className="text-xl max-sm:text-lg font-black mb-7 text-center">Login</p>
          ...
          <div className="grid place-items-center gap-3 mt-6">
            <div className="flex justify-center gap-1 mt-2 text-gray-500 text-xs">
              ...
              <button onClick={() => {onClose(); toggleRegisterModal();}} className="tracking-tighter text-[#60A5FA]">Log in</button>
            </div>
            <h3 className="text-gray-400">- OR -</h3>
            <div className="w-full flex justify-evenly gap-5">
              <button className="flex justify-around items-center gap-2 p-2 border border-gray-500 rounded-2xl text-xs max-[655px]:text-[10px]">
        ...
        <div onClick={(e) => e.stopPropagation()} className={`container w-[533px] max-[1120px]:w-0 h-auto ml-auto p-24 max-[1120px]:p-0 font-poppins rounded-l-3xl rounded-r-lg transition ease-in-out transform`}>
          <p></p>
        </div>
      </div>
    </main>
