...
const _LoginForm: React.FC<LoginFormProps> = ({ onSubmit, error }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <div className="flex flex-col">
        <label htmlFor="email" className='text-gray-500 tex-[15px]'>Password</label>
        <input
          type="password"
      ...
      <button type="submit" className="w-fit place-self-center bg-[#60A5FA] text-white text-xl font-medium py-2 px-24 rounded-lg hover:bg-blue-600">
        Login
      </button>
    </form>
  );
...
const _LoginModal: React.FC<ModalBackdropProps> = ({ open, onClose, toggleRegisterModal }) => {
  ...
  return (
    <main onClick={onClose} className={`fixed inset-0 flex justify-center items-center transition-all duration-300 ease-in-out transform ${open ? "visible bg-black/75 backdrop-blur-md" : "invisible"}`}>
      <div className={`flex justify-center content-center w-fit relative transition-all duration-300 ease-in-out transform  bg-blue-400  ${open ? "scale-100 opacity-100" : "scale-75 opacity-0"}`}>
        <div onClick={(e) => e.stopPropagation()} className={`container w-fit h-fit mr-auto p-24 font-poppins rounded-r-3xl rounded-l-lg bg-white transition-all duration-300 ease-in-out transform`}>
        ...
        <div onClick={(e) => e.stopPropagation()} className={`container w-[533px] h-auto ml-auto p-24 font-poppins rounded-l-3xl rounded-r-lgtransform`}>
          <p></p>
        </div>
      </div>
    </main>
